...
TOKEN = TOKEN = "8653103518:AAHs8a4Qeg4JSDeZ_fDAPk8du5V99nBykVQ"  # <-- вставь сюда токен от BotFather
...
async def button(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if cb == "click":
        ...
        if u["type"] == "click":
            ...
            p["mult_all"] *= u["val"]
        save_data(data)
        cat = "click" if u["type"] == "click" else ("auto" if u["type"] == "auto" else "mult")
        await query.answer(f"Куплено: {u['name']}!")
        await query.edit_message_text(
            make_upgrades_text(p, cat),
            parse_mode="Markdown",
            reply_markup=make_upgrades_keyboard(p, cat),
        )
